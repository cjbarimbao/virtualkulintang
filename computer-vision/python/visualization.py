...
class segmentation(object):
    ...
    def retrieve_patch(self, event, x, y, flags, image):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            I = self.patch.sum(axis=2)
            I[I == 0] = 10000

            self.patch_r = self.patch[:,:,2] / I
            self.patch_g = self.patch[:,:,1] / I
            
            patch_r_int = (self.patch_r*(self.bins-1)).astype(int)
            patch_g_int = (self.patch_g*(self.bins-1)).astype(int)

            self.hmatrix, _, _ = np.histogram2d(patch_g_int.flatten(), patch_r_int.flatten(), bins = self.bins, range = [[0, self.bins], [0, self.bins]])
    ...
    def non_parametric_segmentation(self, frame):
        ...
        self.frame_r_int = (self.frame_r*(bins-1)).astype(int)
        self.frame_g_int = (self.frame_g*(bins-1)).astype(int)

        self.back_projection = np.zeros(self.frame_r.shape, dtype = 'uint8')
        self.back_projection = self.hmatrix1d[self.frame_g_int.flatten()*self.bins + self.frame_r_int.flatten()].reshape(self.frame_r.shape).astype(np.uint8)
        ...
        self.masked = cv2.bitwise_and(frame, frame, mask = self.back_projection)

        return self.back_projection
    
    def non_parametric_slow(self, frame):
            bins = 32
            ...
            self.back_projection2 = np.zeros(self.frame_r.shape, dtype = 'uint8')
            for i in range(self.frame_r.shape[0]):
                for j in range(self.frame_r.shape[1]):
                    self.back_projection2[i,j] = self.hmatrix[self.frame_g_int[i,j], self.frame_r_int[i,j]]
            self.masked2 = cv2.bitwise_and(frame, frame, mask = self.back_projection2)

            return self.back_projection2
    ...
    def main_segmentation(self):
        ...
        while True:
            ...
            mask2 = self.non_parametric_slow(frame)
            ...
            display2 = np.concatenate((frame, self.masked2), axis=0)

            title = "Histogram Backprojection using Matrix Manipulation"
            cv2.namedWindow(title, cv2.WINDOW_NORMAL)
            cv2.resizeWindow(title, self.frame_width, int(self.frame_height*2))
            cv2.imshow(title, display)
            
            title2 = "Histogram Backprojection using For Loops"
            cv2.namedWindow(title2, cv2.WINDOW_NORMAL)
            cv2.resizeWindow(title2, self.frame_width, int(self.frame_height*2))
            cv2.imshow(title2, display2)
...
if __name__ == '__main__':
    ...
    if (save_data):
        ...
        df = []
        x = [start.hmatrix, start.hmatrix1d, start.frame_r_int, start.frame_g_int, start.back_projection, start.back_projection2]
        for data in x:
            df.append(pd.DataFrame(data))

        ## save to xlsx file

        filepath = ('histogram.xlsx', 'hist1d.xlsx', 'r_int.xlsx', 'g_int.xlsx', 'backproj.xlsx', 'backproj2.xlsx')

        for x in range(len(df)):
            df[x].to_excel(filepath[x], index=False)