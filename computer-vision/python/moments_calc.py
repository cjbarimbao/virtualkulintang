...
class segmentation(object):
    ...
    def blob_detection(self, frame):
        ...
        #center = [ np.average(indices) for indices in np.where(thresh >= 255) if (indices).size != 0 ]
        center = [0,0]
    ...
    def main_detection(self):
        ...
        while True:
            ...
            centroid1 = self.blob_detection1(bp_r)
            end = time.time()
            t.append(end - start)
            
            #--------------------------
            moments_mask = self.masked
            if (centroid1.size != 0):
                try:
                    moments_mask = self.disp_centroid(int(centroid1[1]), int(centroid1[0]))
                except: 
                    print(centroid1)
            
            display = np.concatenate((frame, moments_mask), axis = 0)

            title = "Moments Detection"
            cv2.namedWindow(title, cv2.WINDOW_NORMAL)
        ...
        print("Average moments time: {0:.2f} milliseconds".format(avg_t*1000))
